1. ##### 本软件用于生成 '农场AI助手'中 "_internal/script"文件夹内的脚本"数据库文件db_script.db"


   1) 步骤解析原理: 星宝前进的全部步骤的集合为一个tuple, 这个tuple中包含的每一小步骤为dict类型. 程序解析每一小步骤dict, 生成对应的动作.*例如: 无人机v2  = ( {    },{   },{   },... )*
   2) 自定义的tuple经sqlite3的execute()语句存入数据库.
   3) 当然如果熟悉sqlite3, 也可以一步到位, **直接在数据库里修改**, 效果是一样的. 建议使用SQLiteStudio软件打开db_script.db文件.
2. ##### **环境和依赖**

   本文件生成的脚本文件用于'农场AI助手'的版本号>="3.1.0"
...
3. ##### 文件说明:
   ...
   script: 该文件夹包含生成的db_script.db数据库文件
...
5. 数据库文件"db_script.db"说明:

   * "default_"开头的表, 为'有摇杆'模式表单.
   * "notwasd_"开头的表, 为'无摇杆'模式表单.
   * "custom_"开头的表, 为自定义表单.
6. ##### 替换:

   当app"停止脚本"时, 替换或修改"script/db_script.db"中的表单, 数据库表名保持一致, 点击"启动脚本"将运行新脚本.
7. ##### 捕捉农场坐标的方法:
